...
const SymptomBasedGuidanceInputSchema = z.object({
  symptoms: z.string().describe('A detailed description of the experienced symptoms.'),
  ...
  severity: z.string().optional().describe('The severity level of the symptoms (e.g., mild, moderate, severe).'),
});
export type SymptomBasedGuidanceInput = z.infer<typeof SymptomBasedGuidanceInputSchema>;
...
const SymptomBasedGuidanceOutputSchema = z.object({
  possibleCauses: z.array(
    z.object({
      ...
      urgency: z.enum(['routine', 'consult soon', 'emergency']).describe('The recommended urgency level for seeking medical attention.'),
      rationale: z.string().describe('Explanation of why this cause is possible based on the symptoms.')
    })
  ).describe('A ranked list of possible causes for the symptoms, with associated urgency levels and rationales.'),
  disclaimer: z.string().describe('Mandatory disclaimer about the informational nature of the guidance.'),
});
export type SymptomBasedGuidanceOutput = z.infer<typeof SymptomBasedGuidanceOutputSchema>;

export async function symptomBasedGuidance(input: SymptomBasedGuidanceInput): Promise<SymptomBasedGuidanceOutput> {
